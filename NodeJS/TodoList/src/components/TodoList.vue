    ...
    <main>
        <div v-for="(todo,idx) in data" :key="todo.id">
            <div class="input-group my-2 input-group-sm">
                ...
                <div class="input-group-text">
                    <input class="form-input mt-0" type="date" :min="today"
                    disabled :value="todo.date"/>
                </div>
                <input type="text" class="form-control" :value="todo.job"/>
                <button class="btn btn-outline-primary dropdown-toggle"
                type="button" data-bs-toggle="dropdown">할일 관리</button>
...
export default{
    ...
    setup(){
        ...
        const menu=[
            { str:'할일삭제',func:removeTodo},
            { str:'할일완료',func:completeTodo}
        ]
        return { menu,today, completeTodo}
    }
}
